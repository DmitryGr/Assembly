...
static int my_sum(int a, int b, int c, int d) { return a + b + c + d; }
...
static void
read_input(size_t sym_start_offset)
{    
    char buffer[128];
...
static void
call_function_and_print_result(void * addr)
{
	char* ptr = (char*)addr;
...
main()
{
    ...
    // printf("%p", code_buffer);
    // return 0;
    // jit_compile_expression_to_arm(expression_to_parse,
    //                               symbols,
    //                               code_buffer);

    // while (*(char*)code_buffer) {
    // 	printf("%x ", *(char*)code_buffer);
    // 	++code_buffer;
    // }
    printf("%s\n", expression_to_parse);
